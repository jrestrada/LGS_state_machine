...
vertical_height = 10  # Inches
lateral_length = 24   # Inches
end_margin = 2        # Feet
...
def main(args=None):
    ...
    sm.enter_lateral(cmd="forward", times=int(0.8*lateral_length/crawler_rate))
    sm.check_end(cmd = "forward", times = 1)
    sm.retrieve(len = lateral_length)
    while True:
        if sm.is_vert():
            time.sleep(1)
            break
    ...
    sm.transition(len = vertical_height)
    while True:
        if sm.is_hori():
            time.sleep(1)
            break
